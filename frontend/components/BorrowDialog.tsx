...
export default function BorrowDialog({
  ...
  useEffect(() => {
    const fetchMaxBorrowable = async (): Promise<void> => {
      ...
      try {
        ...
        if (typeof window !== "undefined" && window.ethereum) {
          ...
          const [totalDepositedUSD, totalBorrowedUSD] =
            (await lendingPool.getAccountLiquidity(userAddress)) as [
              bigint,
              bigint,
            ];
          const collateralFactor =
            (await lendingPool.collateralFactor()) as bigint;

          const maxBorrowUSD =
            (totalDepositedUSD * collateralFactor) / 10n ** 18n -
            totalBorrowedUSD;
  ...
  const handleBorrow = async (): Promise<void> => {
    ...
    try {
      ...
      setSuccess(
        `Successfully borrowed ${borrowAmount} ${selectedAsset.symbol}`,
      );
      setBorrowAmount("");
        ...
        <Box sx={{ mb: 3 }}>
          <Box sx={{ p: 2, bgcolor: "primary.50", borderRadius: 1, mb: 2 }}>
            ...
            <Typography variant="h6" fontWeight="medium" color="primary.main">
              {selectedAsset &&
                formatAmount(maxBorrowable, selectedAsset.decimals)}{" "}
