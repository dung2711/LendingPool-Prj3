...
export default function Liquidation() {
	...
	const checkUserLiquidatable = async (userAddress) => {
		try {
			...
			const isLiquidatable = liquidation.isAccountLiquidatable(
				healthFactor,
				threshold,
			);
			...
			>
						...
						<>
							<Box sx={{ mb: 3, p: 2, bgcolor: "grey.50", borderRadius: 1 }}>
								...
								<Typography variant="caption" color="text.secondary">
									Health Factor:{" "}
									<Chip
										label={formatHealthFactor(selectedUser.healthFactor)}
										size="small"
										color="error"
									/>
								</Typography>
									...
									selectedAsset.map((asset) => {
										...
										return (
											<Box key={asset.assetAddress} sx={{ mb: 1 }}>
												...
												<Typography variant="caption" color="text.secondary">
													Deposited: {formatAmount(asset.deposited)} | Borrowed:{" "}
				...
				<DialogActions sx={{ px: 3, pb: 2 }}>
					...
					<Button
						...
						startIcon={
							transactionLoading ? <CircularProgress size={20} /> : null
						}
					>
