...
export const proposeUnpause = async () => {
    try {
        ...
        await apiKit.proposeTransaction({
            ...
            safeTransactionData: safeTransaction.data,
            safeTxHash: safeTxHash,
            senderAddress: await (await getProtocolKit()).getSafeProvider().getSignerAddress(),
            senderSignature: signature.data
        });
...
export const executeTransaction = async (safeTxHash) => {
    try {
        ...
        if (safeTransaction.confirmations.length < safeTransaction.confirmationsRequired) {
            throw new Error(`Insufficient signatures. Required: ${safeTransaction.confirmationsRequired}, Got: ${safeTransaction.confirmations.length}`);
        }
...
export const isOwner = async () => {
    try {
        ...
        return owners.map(o => o.toLowerCase()).includes(signerAddress.toLowerCase());
    } catch (error) {
