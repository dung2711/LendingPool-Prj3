...
export const handleDeposit = async (event, blockTimestamp) => {
  try {
    ...
    console.log(
      `Deposit Event: User ${user} deposited ${ethers.formatUnits(amount, 18)} of asset ${asset}`,
    );
...
export const handleWithdraw = async (event, blockTimestamp) => {
  try {
    ...
    console.log(
      `Withdraw Event: User ${user} withdrew ${ethers.formatUnits(amount, 18)} of asset ${asset}`,
    );
...
export const handleBorrow = async (event, blockTimestamp) => {
  try {
    ...
    console.log(
      `Borrow Event: User ${user} borrowed ${ethers.formatUnits(amount, 18)} of asset ${asset}`,
    );
...
export const handleRepay = async (event, blockTimestamp) => {
  try {
    ...
    console.log(
      `Repay Event: User ${user} repaid ${ethers.formatUnits(amount, 18)} of asset ${asset}`,
    );
...
export const handleCollateralSeized = async (event, blockTimestamp) => {
  try {
    ...
    console.log(
      `Liquidation Event: ${ethers.formatUnits(seizeAmount, 18)} collateral seized from ${borrower}`,
    );
    ...
    try {
      ...
      const decimals = await erc20Contract.decimals();
      amountUSD = (
        (seizeAmount * assetPrice) /
        10n ** BigInt(decimals)
      ).toString();
    } catch (error) {
      console.warn(
        `Could not fetch price for asset ${collateralAsset}:`,
        error.message,
      );
    }
...
export const handleAccrue = async (event, blockTimestamp) => {
  try {
    ...
    console.log(
      `Accrue Event: Asset ${asset} - Deposits: ${ethers.formatUnits(newTotalDeposits, 18)}, Borrows: ${ethers.formatUnits(newTotalBorrows, 18)}`,
    );
...
export const handleMarketSupported = async (event, blockTimestamp) => {
  try {
    ...
    console.log(
      `MarketSupported Event: Asset ${asset} with interest model ${interestRateModel}`,
    );
  } catch (error) {
  ...
  try {
      ...
      try {
        ...
        if (isLiquidatable) {
          ...
          if (existingAddresses.has(userAddress)) {
            ...
            console.log(
              `❌ User ${userAddress} is no longer liquidatable (removed)`,
            );
          }
        }
      } catch (error) {
        console.error(
          `Error checking liquidatability for user ${userAddress}:`,
          error,
        );
      }
    }
    ...
    for (const existingUser of existingLiquidatable) {
      if (!currentActiveSet.has(existingUser.userAddress)) {
        ...
        console.log(
          `🔄 User ${existingUser.userAddress} no longer has borrows (removed)`,
        );
      }
    }

    console.log(
      `Liquidatable users calculation completed: ${currentLiquidatableAddresses.length} users`,
    );
