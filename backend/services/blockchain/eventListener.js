...
class EventListener extends EventEmitter {
  ...
  setupEventHandlers() {
    ...
    this.lendingPoolEventHandlers.set(
      "CollateralSeized",
      handleCollateralSeized,
    );
    ...
    this.liquidationEventHandlers.set(
  ...
  async initialize() {
    try {
      ...
      console.log(
        `✅ LendingPool contract: ${await this.lendingPoolContract.getAddress()}`,
      );
      console.log(
        `✅ Liquidation contract: ${await this.liquidationContract.getAddress()}`,
      );
  ...
  async start() {
    ...
    if (
      !this.provider ||
      !this.lendingPoolContract ||
      !this.liquidationContract
    ) {
    ...
    try {
      ...
      this.provider.on("block", async (blockNumber) => {
        try {
          ...
          if (
            blockNumber - this.lastLiquidatableCheck >=
            this.liquidatableCheckInterval
          ) {
            this.lastLiquidatableCheck = blockNumber;
            console.log(
              `🔍 Checking liquidatable users at block ${blockNumber}...`,
            );
  ...
  async getStatus() {
    return {
      ...
      lendingPoolContract: this.lendingPoolContract
        ? await this.lendingPoolContract.getAddress()
        : null,
      liquidationContract: this.liquidationContract
        ? await this.liquidationContract.getAddress()
        : null,
      eventsMonitored: Array.from(this.lendingPoolEventHandlers.keys()).concat(
        Array.from(this.liquidationEventHandlers.keys()),
      ),
      ...
      for (let start = fromBlock; start <= endBlock; start += batchSize) {
        ...
        for (const [eventName, handler] of this.lendingPoolEventHandlers) {
          try {
            ...
            const events = await syncLendingPoolContract.queryFilter(
              filter,
              start,
              end,
            );
            ...
            if (events.length > 0) {
              console.log(
                `   Found ${events.length} ${eventName} events (LendingPool)`,
              );
            }
          } catch (error) {
            console.error(
              `   Error syncing LendingPool ${eventName}:`,
              error.message,
            );
          }
        }
        ...
        for (const [eventName, handler] of this.liquidationEventHandlers) {
          try {
            ...
            const events = await syncLiquidationContract.queryFilter(
              filter,
              start,
              end,
            );
            ...
            if (events.length > 0) {
              console.log(
                `   Found ${events.length} ${eventName} events (Liquidation)`,
              );
            }
          } catch (error) {
            console.error(
              `   Error syncing Liquidation ${eventName}:`,
              error.message,
            );
          }
        }
        ...
        console.log(
          `   Progress: ${progress}% (${processedBlocks}/${endBlock - fromBlock + 1} blocks)`,
        );
      }
