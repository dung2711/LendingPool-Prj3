...
describe("LendingPoolProtocol", () => {
  ...
  it("sets up MyToken parameters correctly and can mint properly", async () => {
    ...
    expect(await priceRouter.getPrice(myToken)).to.equal(
  ...
  it("sets up PriceRouter and MyOracle parameters correctly and they behave as expected", async () => {
    ...
    expect(await myOracle.getPriceMyOracle(myToken)).to.equal(
      ethers.parseUnits("1", 18),
    );
    expect(await priceRouter.getPrice(myToken)).to.equal(
      ethers.parseUnits("1", 18),
    ...
    expect(tx)
      ...
      .withArgs(myToken, ethers.parseUnits("2", 18));
  ...
  it("sets up InterestRateModel parameters correctly", async () => {
    ...
    expect(await interestRateModel.lendingPool()).to.equal(
  ...
  it("sets up LendingPool parameters correctly", async () => {
    ...
    expect(tokenMarket.interestRateModel).to.equal(
  ...
  it("sets up Liquidation parameters correctly", async () => {
    expect(await liquidation.priceRouter()).to.equal(
      await priceRouter.getAddress(),
    );
    expect(await liquidation.lendingPool()).to.equal(
      await lendingPool.getAddress(),
    );
    expect(await liquidation.liquidationThreshold()).to.equal(
      ethers.parseUnits("0.9", 18),
    );
    expect(await liquidation.closeFactor()).to.equal(
      ethers.parseUnits("0.5", 18),
    );
    expect(await liquidation.liquidationIncentive()).to.equal(
      ethers.parseUnits("0.05", 18),
    );
  });

  it("can support and unsupport markets properly", async () => {
    const tx = await lendingPool.supportMarket(
    ...
    const market = await lendingPool.markets(
      "0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238",
    );
    expect(market.isSupported).to.equal(true);
    expect(market.interestRateModel).to.equal(interestRateModel);
    expect(
      await lendingPool.marketExists(
        "0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238",
      ),
    ).to.equal(true);
    const tx2 = await lendingPool.unsupportMarket(
      "0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238",
    );
    expect(tx2)
      .to.emit(lendingPool, "MarketUnsupported")
      .withArgs("0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238");
    const market2 = await lendingPool.markets(
      "0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238",
    );
  ...
  it("can deposit, borrow, withdraw and repay properly", async () => {
    ...
    expect(tx)
      ...
      .withArgs(owner.address, myToken, ethers.parseUnits("1000", 18));
    const marketAfterDeposit = await lendingPool.markets(myToken);
    expect(marketAfterDeposit.totalDeposits).to.equal(
      ethers.parseUnits("1000", 18),
    );
    expect(await myToken.balanceOf(owner.address)).to.equal(
    ...
    expect(
      (await lendingPool.userBalances(owner.address, myToken)).deposited,
    ).to.equal(ethers.parseUnits("1000", 18));
    expect(await lendingPool.userMarkets(owner.address, 0)).to.equal(myToken);
    expect(await lendingPool.userMarketExists(owner.address, myToken)).to.equal(
      true,
    );
    expect(
      (await lendingPool.userBalances(owner.address, myToken))
        .depositIndexSnapShot,
    ).to.equal(marketAfterDeposit.depositIndex);

    //Borrow
    await expect(
      lendingPool.borrow(myToken, ethers.parseUnits("900", 18)),
    ).to.be.revertedWith("Insufficient collateral");
    const tx2 = await lendingPool.borrow(myToken, ethers.parseUnits("500", 18));
    expect(tx2)
      .to.emit(lendingPool, "Borrow")
      .withArgs(owner.address, myToken, ethers.parseUnits("500", 18));
    const marketAfterBorrow = await lendingPool.markets(myToken);
    expect(marketAfterBorrow.totalBorrows).to.equal(
      ethers.parseUnits("500", 18),
    );
    expect(await myToken.balanceOf(owner.address)).to.equal(
      ethers.parseUnits("999500", 18),
    );
    expect(
      (await lendingPool.userBalances(owner.address, myToken)).borrowed,
    ).to.equal(ethers.parseUnits("500", 18));
    expect(
      (await lendingPool.userBalances(owner.address, myToken))
        .borrowIndexSnapShot,
    ).to.equal(marketAfterBorrow.borrowIndex);
    ...
    expect(tx3)
      ...
      .withArgs(owner.address, myToken, ethers.parseUnits("200", 18));
    const marketAfterRepay = await lendingPool.markets(myToken);
    expect(marketAfterRepay.totalBorrows).to.be.closeTo(
      ethers.parseUnits("300", 18),
      ethers.parseUnits("0.001", 18),
    );
    expect(await myToken.balanceOf(owner.address)).to.equal(
      ethers.parseUnits("999300", 18),
    );
    expect(
      (await lendingPool.userBalances(owner.address, myToken)).borrowed,
    ).to.be.closeTo(
    ...
    expect(
      (await lendingPool.userBalances(owner.address, myToken))
        .borrowIndexSnapShot,
    ).to.equal(marketAfterRepay.borrowIndex);

    //Withdraw partially
    const tx5 = await lendingPool.withdraw(
      myToken,
      ethers.parseUnits("400", 18),
    );
    expect(tx5)
      .to.emit(lendingPool, "Withdraw")
      .withArgs(owner.address, myToken, ethers.parseUnits("400", 18));
    const marketAfterWithdraw = await lendingPool.markets(myToken);
    expect(marketAfterWithdraw.totalDeposits).to.be.closeTo(
      ethers.parseUnits("600", 18),
      ethers.parseUnits("0.001", 18),
    );
    expect(await myToken.balanceOf(owner.address)).to.equal(
      ethers.parseUnits("999700", 18),
    );
    expect(
      (await lendingPool.userBalances(owner.address, myToken)).deposited,
    ).to.be.closeTo(
    ...
    expect(
      (await lendingPool.userBalances(owner.address, myToken))
        .depositIndexSnapShot,
    ).to.equal(marketAfterWithdraw.depositIndex);
  });
  ...
  it("can accrue interest properly", async () => {
    ...
    const _tx = await lendingPool.accrueInterest(myToken);

    // Manually calculate expected rates

    expect(borrowRate).to.equal(
      baseRate +
        (rateSlope1 *
          ((marketBefore.totalBorrows * 1_000_000_000_000_000_000n) /
            marketBefore.totalDeposits)) /
          1_000_000_000_000_000_000n,
    );
    expect(depositRate).to.equal(
      (borrowRate *
        ((marketBefore.totalBorrows * 1_000_000_000_000_000_000n) /
          marketBefore.totalDeposits) *
        (1_000_000_000_000_000_000n - reserveFactor)) /
        1_000_000_000_000_000_000n /
        1_000_000_000_000_000_000n,
    );
    ...
    expect(marketAfter.totalBorrows).to.be.closeTo(
      expectedTotalBorrowsIncrease + marketBefore.totalBorrows,
      ethers.parseUnits("0.01", 18),
    );
    expect(marketAfter.totalDeposits).to.be.closeTo(
      expectedTotalDepositsIncrease + marketBefore.totalDeposits,
      ethers.parseUnits("0.01", 18),
    );
    expect(marketAfter.borrowIndex).to.be.closeTo(
      expectedBorrowIndexIncrease + marketBefore.borrowIndex,
      ethers.parseUnits("0.0001", 18),
    );
    expect(marketAfter.depositIndex).to.be.closeTo(
      expectedDepositIndexIncrease + marketBefore.depositIndex,
      ethers.parseUnits("0.0001", 18),
    );
  });
  ...
  it("can liquidate properly", async () => {
    ...
    await myToken
      .connect(addr1)
      .approve(lendingPool, ethers.parseUnits("1000", 18));
    await lendingPool
      .connect(addr1)
      .deposit(myToken, ethers.parseUnits("1000", 18));
    expect(
      (await lendingPool.userBalances(addr1.address, myToken)).deposited,
    ).to.equal(ethers.parseUnits("1000", 18));

    //Addr1 borrows 700 MyToken
    await lendingPool
      .connect(addr1)
      .borrow(myToken, ethers.parseUnits("700", 18));

    //set liquidation threshold low to allow liquidation
    await liquidation.setLiquidateParams(
      ethers.parseUnits("0.5", 18),
      ethers.parseUnits("0.5", 18),
      ethers.parseUnits("0.05", 18),
    );
    ...
    await myToken
      .connect(owner)
      .approve(liquidation, ethers.parseUnits("350", 18));
    const tx = await liquidation
      .connect(owner)
      .liquidate(addr1, owner, myToken, myToken, ethers.parseUnits("350", 18));
    expect(tx)
      .to.emit(liquidation, "Liquidation")
      .withArgs(
        owner.address,
        addr1.address,
        myToken,
        ...
        ethers.parseUnits("367.5", 18),
      );

    //Check balances after liquidation
    const addr1Balances = await lendingPool.userBalances(
      addr1.address,
      myToken,
    );
    expect(addr1Balances.borrowed).to.be.closeTo(
      ethers.parseUnits("350", 18),
      ethers.parseUnits("0.001", 18),
    );
    expect(addr1Balances.deposited).to.be.closeTo(
      ethers.parseUnits("632.5", 18),
      ethers.parseUnits("0.001", 18),
    );
    const ownerBalances = await myToken.balanceOf(owner.address);
    expect(ownerBalances).to.be.closeTo(
      ethers.parseUnits("999017.5", 18),
      ethers.parseUnits("0.001", 18),
    );
  });
});
